    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            Canvas { context, size in
                ...
                for index in 0 ..< adjustedPoints.count {
                    let next = (index + 1) % adjustedPoints.count

                    // Calculate the angle between points
                    let currentAngle = atan2(
                        adjustedPoints[index].y - center.y,
                        adjustedPoints[index].x - center.x
                    )
                    let nextAngle = atan2(
                    ...
                    let control1 = CGPoint(
                        x: adjustedPoints[index].x + cos(currentAngle + .pi / 2) * handleLength,
                        y: adjustedPoints[index].y + sin(currentAngle + .pi / 2) * handleLength
                    )
